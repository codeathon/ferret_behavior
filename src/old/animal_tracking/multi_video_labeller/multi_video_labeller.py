...
from src.animal_tracking.multi_video_labeller.helpers.multi_video_processor import (
    MultiVideoProcessor,
)
...
class MultiVideoLabeller(BaseModel):
    ...
    def create(
        ...
        data_handler_config: (
            str | Path
        ) = "src/animal_tracking/multi_video_labeller/helpers/face_points.json",
    ):
        video_processor = MultiVideoProcessor.from_folder(
            video_folder, max_window_size, data_handler_config
        )
        return cls(
    ...
    def _handle_keypress(self, key: int):
        if key == 27:  # ESC
            ...
            self.video_processor.image_annotator.config.show_help = self.show_help
        elif key == ord("u"):
            self.video_processor.data_handler.clear_current_point(self.frame_number)
        return True
    ...
    def _mouse_callback(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            self.video_processor.handle_clicks(
        ...
        try:
            while True:
                ...
                if not self._handle_keypress(key):
                    break
                grid_image = self.video_processor.create_grid_image(
